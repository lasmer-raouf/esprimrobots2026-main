...
export function MemberCompetition() {
  ...
  const handleSignup = (robotId: number) => {
    const robot = clubDB.competitionRobots.find(r => r.id === robotId);
    if (!robot) return;

    if (robot.signups.includes(user.id)) {
      // Remove signup
      robot.signups = robot.signups.filter(id => id !== user.id);
      toast({
        title: 'Slot Released',
      ...
      // Add signup
      if (robot.signups.length >= robot.slots) {
        toast({
          title: 'Slot Full',
